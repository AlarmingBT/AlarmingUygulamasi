...
public class DailyQuote extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM dailyMotivations where id = " + random,null);


        while(!cursor.moveToFirst()) {
            random = (new Random()).nextInt(10);
            cursor = db.rawQuery("SELECT * FROM dailyMotivations where id = " + random, null);
        }

        setContentView(R.layout.activity_daily_quote);
        mQuote = (TextView) findViewById(R.id.txt_quote);
        mAuthor = (TextView) findViewById(R.id.author);
        mVisible = true;
        mControlsView = findViewById(R.id.fullscreen_content_controls);
        mContentView = findViewById(R.id.fullscreen_content);

        mQuote.setText(cursor.getString(1));
        mAuthor.setText(cursor.getString(2));
